...
function MatchCard({ match, onAccept, onReject, onCancel }) {
  ...
  const labelStyles = {
    ...
    canceled: "border-slate-500/20 bg-slate-500/10 text-slate-200",
  };
  ...
  return (
    <div className="rounded-[24px] border border-white/10 bg-slate-900/70 p-6 shadow-[0_20px_70px_-34px_rgba(0,0,0,0.95)] transition hover:border-violet-400/30">
      <div className="flex items-start justify-between gap-4">
        ...
        <span className={`rounded-full border px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.18em] ${labelStyles[match.status] || "border-violet-400/20 bg-violet-500/10 text-violet-200"}`}>
          {match.status}
        </span>
      </div>
      ...
      {match.status === "pending" && match.type === "received" && (
        <div className="mt-6 flex gap-3">
          <button type="button" onClick={() => onAccept(match.id || match._id)} className="flex flex-1 items-center justify-center gap-2 rounded-2xl bg-emerald-600/90 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-emerald-500">
            <FiCheck /> Accept
          </button>
          <button type="button" onClick={() => onReject(match.id || match._id)} className="flex flex-1 items-center justify-center gap-2 rounded-2xl bg-rose-600/90 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-rose-500">
            <FiX /> Reject
          </button>
        </div>
      )}
      ...
      {match.status === "pending" && match.type === "sent" && (
        <button type="button" onClick={() => onCancel(match.id || match._id)} className="mt-6 flex w-full items-center justify-center gap-2 rounded-2xl border border-amber-400/20 bg-amber-500/10 px-4 py-2.5 text-sm font-semibold text-amber-200 transition hover:bg-amber-500/20">
